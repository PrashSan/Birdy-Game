...
class Game:
    def __init__(self):
        ...
        self.score=0
        self.font=pg.font.Font("assets/font.ttf",25)
        ...
        self.restart_text_rect=self.score_text.get_rect(center=(300,600))
        ...
        while True:
            ...
            for event in pg.event.get():
                if event.type == pg.QUIT:
                    pg.quit()
                    sys.exit()
                if event.type==pg.KEYDOWN and self.isgameStarted:
                    ...
                    if event.key==pg.K_SPACE and self.is_enter_pressed:
                        ...
                        ## to know wheter restart button has been presssed or not
    ...
    def checkscore(self):
        if len(self.pipes) >0:
            if self.bird.rect.left>self.pipes[0].rect_down.left and self.bird.rect.right <self.pipes[0].rect_down.right and not self.start_monitring:
                self.start_monitring=True

            if self.bird.rect.left >  self.pipes[0].rect_down.right and self.start_monitring:
                self.start_monitring=False
                self.score+=1
